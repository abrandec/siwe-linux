...
use crate::misc::{conv, enums::*, functions::*, types::*};
...
/// use pam::Client;
///
/// let mut client = Client::start("system-auth")
///         .expect("Failed to init PAM client.");
/// // Preset the login & password we will use for authentication
/// client.conversation_mut().set_credentials("user", "signature");
...
impl<'a> Client<'a, conv::SiweConv> {
    /// Create a new `Client` with the given service name and a signature-based conversation
    pub fn start(service: &str) -> PamResult<Client<'a, conv::SiweConv>> {
        Client::with_conversation(service, conv::SiweConv::new())
    }
}
