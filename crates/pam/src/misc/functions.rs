...
#[cfg(feature = "client")]
mod appl {
    use crate::misc::{ffi, PamFlag, PamHandle, PamResult, PamReturnCode};
...
mod types {
    use crate::misc::{env, ffi, PamHandle, PamItemType, PamResult, PamReturnCode};
...
#[cfg(target_os = "linux")]
mod misc {
    use crate::misc::{ffi, PamHandle, PamResult, PamReturnCode};
...
#[inline]
fn buffer_error<T>() -> crate::misc::PamResult<T> {
    Err(crate::misc::PamReturnCode::Buf_Err.into())
}

fn try_str_option_to_ptr(opt: Option<&str>) -> crate::misc::PamResult<*const libc::c_char> {
    match opt.map(std::ffi::CString::new) {
        ...
        _ => Err(crate::misc::PamReturnCode::Buf_Err.into()),
    }
}
