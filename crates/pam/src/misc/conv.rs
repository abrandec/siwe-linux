...
use crate::misc::{ffi::pam_conv, PamMessage, PamMessageStyle, PamResponse, PamReturnCode};
...
/// `SiweConv` implementation provided by this crate.
...
impl SiweConv {
    /// Create a new `SiweConv` handler
    pub(crate) fn new() -> SiweConv {
        SiweConv {
            user: String::new(),
    ...
    pub fn set_credentials<U: Into<String>, V: Into<String>>(&mut self, user: U, signature: V) {
        self.user = user.into();
        self.signature = signature.into();
    }
}

impl Conversation for SiweConv {
    fn prompt_echo(&mut self, _msg: &CStr) -> Result<CString, ()> {
        CString::new(self.user.clone()).map_err(|_| ())
    }
    fn prompt_blind(&mut self, _msg: &CStr) -> Result<CString, ()> {
        CString::new(self.signature.clone()).map_err(|_| ())
    }
